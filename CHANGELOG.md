# Changelog


## v0.3.1

[compare changes](https://github.com/erikkkwu/protomend/compare/v0.3.0...v0.3.1)

### 🩹 Fixes

- **ui:** Rename missed REHEADER wordmark to PROTOMEND ([41785d3](https://github.com/erikkkwu/protomend/commit/41785d3))

### 🏡 Chore

- Gitignore history folder ([e3ac0b3](https://github.com/erikkkwu/protomend/commit/e3ac0b3))

### ❤️ Contributors

- Erikwu ([@erikkkwu](https://github.com/erikkkwu))

## v0.3.0

[compare changes](https://github.com/erikkkwu/protomend/compare/v0.2.1...v0.3.0)

### 🏡 Chore

- Add font OFL license notices and tighten gitignore ([eeec903](https://github.com/erikkkwu/protomend/commit/eeec903))
- Rename project to Protomend ([bfe8834](https://github.com/erikkkwu/protomend/commit/bfe8834))

### ❤️ Contributors

- Erikwu ([@erikkkwu](https://github.com/erikkkwu))

## v0.2.1

[compare changes](https://github.com/erikkkwu/reheader/compare/v0.2.0...v0.2.1)

### 🚀 Enhancements

- **background:** Show off badge when profile is disabled ([6fc0343](https://github.com/erikkkwu/reheader/commit/6fc0343))
- **background:** Use red for off badge ([f2f0413](https://github.com/erikkkwu/reheader/commit/f2f0413))
- **background:** Gray out toolbar icon when profile is disabled ([010c769](https://github.com/erikkkwu/reheader/commit/010c769))

### ❤️ Contributors

- Erikwu ([@erikkkwu](https://github.com/erikkkwu))

## v0.2.0

[compare changes](https://github.com/erikkkwu/reheader/compare/v0.1.0...v0.2.0)

### 🚀 Enhancements

- **profile:** Add Duplicate action to clone the selected profile ([b5f08fc](https://github.com/erikkkwu/reheader/commit/b5f08fc))

### 🏡 Chore

- Adopt conventional commits and changelogen ([88b2aa1](https://github.com/erikkkwu/reheader/commit/88b2aa1))

### ❤️ Contributors

- Erikwu ([@erikkkwu](https://github.com/erikkkwu))

